...
export class WeiboAdapter extends BasePlatformAdapter {
  ...
  async publish(post) {
    ...
    // 设置任务标签和步骤
    this._overlayTaskLabel = '微博 · 动态发布任务执行中'
    const hasImages = post.images && post.images.length > 0
    const steps = ['预热浏览', '打开发微博页面', '输入内容']
    ...
    try {
      S++
      await this.showStatus('正在模拟人工预热浏览', { next: '打开发微博页面', step: S, total: T }).catch(() => {})
      await this.warmupBrowse()

      S++
      await this.showStatus('正在打开微博发布编辑器', { next: '输入内容', step: S, total: T }).catch(() => {})
      await this.runStep('openPublishPage', () => this.step1_openPublishPage())
      S++
      await this.showStatus('正在模拟人工输入微博内容', { next: hasImages ? '上传图片' : '发送微博', step: S, total: T }).catch(() => {})
      await this.runStep('inputContent', () => this.step2_inputContent(post.content || post.title || ''))

      if (hasImages) {
        S++
        await this.showStatus('正在上传图片素材并等待处理', { next: '发送微博', step: S, total: T }).catch(() => {})
        await this.runStep('uploadImages', () => this.step3_uploadImages(post.images))
      }

      S++
      await this.showStatus('正在点击发送按钮发布微博', { step: S, total: T }).catch(() => {})
