...
export class SohuAdapter extends BasePlatformAdapter {
  ...
  async publish(post) {
    ...
    // 设置任务标签和步骤
    this._overlayTaskLabel = '搜狐号 · 文章发布任务执行中'
    const hasCover = post.images && post.images.length > 0
    const steps = ['预热浏览', '打开编辑器', '输入标题', '输入正文', '输入摘要', '设置原创声明', '设置信息来源', '设置话题']
    ...
    try {
      ...
      S++
      await this.showStatus('正在打开搜狐号文章编辑器', { next: '输入标题', step: S, total: T }).catch(() => {})
      ...
      S++
      await this.showStatus('正在模拟人工输入正文内容', { next: '输入摘要', step: S, total: T }).catch(() => {})
      ...
      S++
      await this.showStatus('正在设置原创声明', { next: '设置信息来源', step: S, total: T }).catch(() => {})
      await this.runStep('setOriginal', () => this.step5_setOriginal())
      S++
      await this.showStatus('正在设置信息来源', { next: '设置话题', step: S, total: T }).catch(() => {})
      await this.runStep('setInfoSource', () => this.step5b_setInfoSource(post.infoSource || 'ai'))
      S++
      await this.showStatus('正在设置话题标签', { next: hasCover ? '上传封面图' : '发布文章', step: S, total: T }).catch(() => {})
