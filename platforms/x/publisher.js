...
export class XAdapter extends BasePlatformAdapter {
  ...
  async publish(post) {
    ...
    try {
      // 设置任务标签和步骤
      this._overlayTaskLabel = 'X · 推文发布任务执行中'
      const hasImages = post.images && post.images.length > 0
      const steps = ['预热浏览', '打开发推页面', '输入推文内容']
      if (hasImages) steps.push('上传图片')
      steps.push('AI 视觉验证', '发布推文')
      const T = steps.length
      let S = 0

      // 发帖前预热浏览：先浏览首页 feed，建立自然行为链
      S++
      await this.showStatus('正在模拟人工预热浏览', { next: '打开发推页面', step: S, total: T }).catch(() => {})
      await this.warmupBrowse()

      S++
      await this.showStatus('正在打开 X 发推编辑器', { next: '输入推文内容', step: S, total: T }).catch(() => {})
      await this.runStep('openPage', () => this.step1_openPage())

      // X 无标题，用 content 或 fallback 到 title
      const text = post.content || post.title || ''
      S++
      await this.showStatus('正在模拟人工输入推文内容', { next: hasImages ? '上传图片' : 'AI 视觉验证', step: S, total: T }).catch(() => {})
      await this.runStep('inputContent', () => this.step2_inputContent(text))

      if (hasImages) {
        S++
        await this.showStatus('正在上传图片素材并等待处理', { next: 'AI 视觉验证', step: S, total: T }).catch(() => {})
        await this.runStep('uploadImages', () => this.step3_uploadImages(post.images))
      }

      // AI 视觉验证：发布前截图确认内容正确
      S++
      await this.showStatus('正在进行 AI 视觉验证截图', { next: '发布推文', step: S, total: T }).catch(() => {})
      ...
      S++
      await this.showStatus('正在点击发布按钮发送推文', { step: S, total: T }).catch(() => {})
  ...
  async step1_openPage() {
    ...
    // DOM 层 editor ready 三元组兜底（用户三点要求 #2：每步可判断）
    // composeDialog 已检测；这里再做 URL/text 三元组的形式化记录（便于后续审计）
    await this.assertEditorReady(STATE_INDICATORS.editor, { throwOnFail: false, timeoutMs: 5000 })
  ...
  async step4_submit() {
    ...
    if (!el) {
      ...
      if (visionClick.clicked) {
        ...
        if (published) {
          ...
          // 视觉判断发布结果
          const vr = await this.visionCheckPublishResult({ platformName: 'X (Twitter)' })
          if (vr.status === 'failed') {
            await this.conditionalScreenshot('x_vision_failed', 'error')
            throw new Error(`X 发布失败（视觉判定）: ${vr.details}`)
          }
          return
        }
      }
    ...
    if (publishResult.status === 'failed') {
      await this.conditionalScreenshot('x_vision_failed', 'error')
      throw new Error(`X 发布失败（视觉判定）: ${publishResult.details}`)
    }
  }
