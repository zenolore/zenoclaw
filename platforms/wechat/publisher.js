...
export class WechatAdapter extends BasePlatformAdapter {
  ...
  async publish(post) {
    ...
    try {
      ...
      // 设置任务标签和步骤
      this._overlayTaskLabel = '微信公众号 · 文章发布任务执行中'
      const hasCover = normalizedPost.images && normalizedPost.images.length > 0
      const steps = ['预热浏览', '打开编辑器', '输入标题', '输入正文', '输入作者', '输入摘要', '设置原创声明']
      if (hasCover) steps.push('上传封面图')
      steps.push('模拟人工通读检查', '发表文章')
      ...
      S++
      await this.showStatus('正在打开公众号文章编辑器', { next: '输入标题', step: S, total: T }).catch(() => {})
      ...
      S++
      await this.showStatus('正在模拟人工输入正文内容', { next: '输入作者', step: S, total: T }).catch(() => {})
      ...
      S++
      await this.showStatus('正在输入作者信息', { next: '输入摘要', step: S, total: T }).catch(() => {})
      await this.runStep('inputAuthor', () => this.step4_inputAuthor(normalizedPost.author || ''))

      S++
      await this.showStatus('正在输入文章摘要', { next: '设置原创声明', step: S, total: T }).catch(() => {})
      await this.runStep('inputDigest', () => this.step5_inputDigest(normalizedPost.summary || normalizedPost.content.substring(0, 80)))

      S++
      await this.showStatus('正在设置原创声明', { next: hasCover ? '上传封面图' : '通读检查', step: S, total: T }).catch(() => {})
      ...
      S++
      await this.showStatus('正在模拟人工通读检查内容', { next: '发表文章', step: S, total: T }).catch(() => {})
      await this.runStep('reviewBeforeSubmit', () => this.reviewBeforeSubmit())

      S++
      await this.showStatus('正在点击发表按钮提交文章', { step: S, total: T }).catch(() => {})
