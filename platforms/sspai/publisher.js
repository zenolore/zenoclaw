...
export class SspaiAdapter extends BasePlatformAdapter {
  ...
  async publish(post) {
    ...
    // 设置任务标签和步骤
    this._overlayTaskLabel = '少数派 · 文章发布任务执行中'
    const hasCover = post.images && post.images.length > 0
    const steps = ['预热浏览', '打开写文章页', '输入标题', '输入正文']
    ...
    try {
      S++
      await this.showStatus('正在模拟人工预热浏览', { next: '打开写文章页', step: S, total: T }).catch(() => {})
      await this.warmupBrowse()

      S++
      await this.showStatus('正在打开少数派写文章页面', { next: '输入标题', step: S, total: T }).catch(() => {})
      ...
      S++
      await this.showStatus('正在模拟人工输入正文内容', { next: hasCover ? '上传封面图' : '发布文章', step: S, total: T }).catch(() => {})
  ...
  async step1_openPage() {
    ...
    // 编辑器就绪检测：URL=/write + 「发布」按钮 + 标题输入框三元组（实测 2026-05-04）
    await this.assertEditorReady(STATE_INDICATORS.editor)
  ...
  async step5_publish() {
    ...
    // 发布成功判断（用户三点要求 #3：必须真实命中信号，不能"没发布还说发布了"）
    // 三元组（任一命中即视为成功）：
    //   - URL 跳转：/write → /post/{id} 或 /matrix
    //   - selector：.el-message--success / .el-notification--success（element ui toast）
    //   - 文本："发布成功" / "已发布" / "提交成功"
    const successResult = await this.waitForPublishSuccess(STATE_INDICATORS.publishOk, {
      timeoutMs: 25000,
      pollMs: 1000
    })
    if (!successResult.ready) {
      // 未命中：保守做法是抛错让上游知道发布结果不确定（不能盲目报成功）
      await this.conditionalScreenshot('sspai_publish_unconfirmed', 'error')
      throw new Error(
        `少数派发布结果未确认：URL/selector/text 三元组均未命中（${successResult.ms}ms 超时）。`
        + '可能：发布失败 / 进入审核状态 / 页面结构变更。请人工核查最新一次截图。'
      )
    }
    this.log.info(`[步骤5] 发布成功确认：${JSON.stringify(successResult.hits)}`)
  }
