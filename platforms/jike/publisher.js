...
import { PUBLISH_SELECTORS, INTERACT_SELECTORS, STATE_INDICATORS } from './selectors.js'
...
export class JikeAdapter extends BasePlatformAdapter {
  ...
  async publish(post) {
    ...
    // 设置任务标签和步骤
    this._overlayTaskLabel = '即刻 · 动态发布任务执行中'
    const hasImages = post.images && post.images.length > 0
    const steps = ['预热浏览', '打开即刻首页', '输入动态内容']
    ...
    try {
      S++
      await this.showStatus('正在模拟人工预热浏览', { next: '打开即刻首页', step: S, total: T }).catch(() => {})
      await this.warmupBrowse()

      S++
      await this.showStatus('正在打开即刻首页编辑器', { next: '输入动态内容', step: S, total: T }).catch(() => {})
      await this.runStep('openPage', () => this.step1_openPage())

      S++
      await this.showStatus('正在模拟人工输入动态内容', { next: hasImages ? '上传图片' : '发送动态', step: S, total: T }).catch(() => {})
      await this.runStep('inputContent', () => this.step2_inputContent(post.content || post.title))

      if (hasImages) {
        S++
        await this.showStatus('正在上传图片素材并等待处理', { next: '发送动态', step: S, total: T }).catch(() => {})
        await this.runStep('uploadImages', () => this.step3_uploadImages(post.images))
      }

      S++
      await this.showStatus('正在点击发送按钮', { step: S, total: T }).catch(() => {})
  ...
  async step1_openPage() {
    ...
    // 编辑器就绪检测（jike 主页即编辑器，URL/selector/text 三元组任一命中即视为 ready）
